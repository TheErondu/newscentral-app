...
class BlogNews {
  ...
  BlogNews.fromJson(Map<String, dynamic> parsedJson) {
    try {
      ...
      if (imageFeature == null &&
          parsedJson['jetpack_featured_media_url'] != null) {
        imageFeature = parsedJson['jetpack_featured_media_url'];
      }
...
//      author = parsedJson["_embedded"]["author"][0]["name"];
  ...
  BlogNews.fromPageJson(Map<String, dynamic> parsedJson) {
    try {
      id = parsedJson["id"];
      slug = parsedJson["slug"];
      title = HtmlUnescape().convert(parsedJson["title"]["rendered"]);
...
//
  ...
  @override
  String toString() =>
      'Blog { id: $id date: $date name: $title cateid $categoryId}';
}
