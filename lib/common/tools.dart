...
class Tools {
  ...
  static String getTime(String time, {context}) {
    ...
    if (now.difference(date).inDays > 0) {
      ...
      if (timeDifInDay < 30) {
        return '${S.of(context).daysAgo(timeDifInDay)}';
      } else if (timeDifInDay < 365) {
        return '${S.of(context).monthsAgo(timeDifInDay ~/ 12)}';
      } else {
        return '${S.of(context).yearsAgo(timeDifInDay ~/ 365)}';
      }
    } else if (now.difference(date).inHours > 0) {
      return '${S.of(context).hoursAgo(now.difference(date).inHours)}';
    } else if (now.difference(date).inMinutes > 0) {
      return '${S.of(context).minutesAgo(now.difference(date).inMinutes)}';
    } else if (now.difference(date).inSeconds > 0) {
      return '${S.of(context).secondsAgo(now.difference(date).inSeconds)}';
    } else {
      return '${S.of(context).momentAgo}';
    }
  }
}
  ...
  static Function getLanguagesList = ([context]) {
    return [
      ...
      },
      {
        ...
        "icon": "assets/images/country/ae.png",
        "code": "ar",
        "text": "Arabic",
        "storeViewCode": "ar" //for magento
      },
      {
        "name": context != null ? S.of(context).turkish : "Turkish",
        "icon": "assets/images/country/tu.png",
        "code": "tr",
        "text": "Turkish",
        "storeViewCode": ""
      },
